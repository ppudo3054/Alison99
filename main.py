...
async def subscription_portal(uuid: str, request: Request):
    async with LINKS_LOCK:
        link = dict(LINKS.get(uuid) or {})
    if not is_link_allowed(link):
        raise HTTPException(status_code=404, detail="subscription not found or inactive")
    host = get_host(request)
    raw_url = f"{get_scheme()}://{host}/sub/{uuid}"
    label = str(link.get("label") or "ALISON ACCESS")
    ...
    initial = escape_html((label.strip()[:1] or "A").upper())
    safe_label = escape_html(label)
    safe_protocol = escape_html(protocol)
    safe_url = escape_html(raw_url)
    status = "ACTIVE" if active else "OFFLINE"
    status_class = "active" if active else "offline"
    active_ips = {str(x.get("ip") or "").strip() for x in connections.values() if x.get("uuid") == uuid and str(x.get("ip") or "").strip()}
...
<meta name="theme-color" content="#05080b"><title>{safe_label} · ALISON</title>
<link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;500;600;700;800;900&family=Space+Grotesk:wght@500;600;700&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@3.34.0/dist/tabler-icons.min.css">
<style>
:root{{--bg:#05080b;--panel:#0a1014;--panel2:#0d1619;--line:#1b2a30;--text:#eef8f5;--muted:#82969d;--soft:#53666e;--a:#42f5c5;--b:#28b9ff;--r:#ff647e}}
*{{box-sizing:border-box}}html,body{{margin:0;background:var(--bg);color:var(--text);font-family:Vazirmatn,sans-serif}}body{{min-height:100vh}}.wrap{{width:min(720px,calc(100% - 24px));margin:auto;padding:18px 0 90px}}.top{{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}}.brand{{display:flex;align-items:center;gap:9px}}.mark{{width:38px;height:38px;border:1px solid #285149;background:#0b1716;border-radius:12px;display:grid;place-items:center;color:var(--a);font:800 15px 'Space Grotesk'}}.brand b{{font:700 14px 'Space Grotesk';letter-spacing:-.04em}}.brand small{{display:block;color:var(--soft);font-size:7px;letter-spacing:.16em}}.live{{padding:7px 10px;border-radius:999px;font-size:8px;border:1px solid rgba(66,245,197,.22);color:var(--a);background:rgba(66,245,197,.06)}}.live.off{{color:var(--r);border-color:rgba(255,100,126,.22);background:rgba(255,100,126,.06)}}
.passport{{position:relative;overflow:hidden;border:1px solid #25423e;border-radius:27px;background:linear-gradient(145deg,#0b1717,#080d11);padding:23px;box-shadow:0 30px 90px rgba(0,0,0,.4)}}.passport:after{{content:"ALISON";position:absolute;left:-28px;bottom:20px;color:rgba(66,245,197,.025);font:900 100px 'Space Grotesk';transform:rotate(-18deg)}}.head{{display:flex;justify-content:space-between;align-items:flex-start;gap:14px}}.identity{{display:flex;align-items:center;gap:12px;min-width:0}}.avatar{{width:58px;height:58px;flex:none;border-radius:18px;background:#0c211e;border:1px solid #28564d;display:grid;place-items:center;color:var(--a);font:800 24px 'Space Grotesk'}}.eyebrow{{font-size:8px;color:var(--a);font-weight:900;letter-spacing:.18em}}h1{{font-size:23px;margin:5px 0 2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}.proto{{color:#60777b;font-size:8px}}.state{{padding:7px 9px;border-radius:8px;font-size:8px;font-weight:900;background:#0b211d;color:var(--a);border:1px solid #1e4e45}}.state.offline{{background:#251016;color:var(--r);border-color:#562331}}.notice{{margin-top:20px;padding:10px 12px;border:1px solid #1a3a34;background:#091512;border-radius:11px;color:#86aaa1;font-size:9px}}.rule{{height:1px;background:#17272c;margin:19px 0}}.usage-head{{display:flex;justify-content:space-between;align-items:end}}.usage-head b{{font-size:10px}}.usage-head span{{font:700 24px 'Space Grotesk';color:var(--a)}}.bar{{height:13px;background:#07100f;border:1px solid #17332f;border-radius:999px;padding:2px;margin:11px 0 13px;overflow:hidden}}.fill{{height:100%;width:{pct}%;max-width:100%;background:linear-gradient(90deg,#22ad8e,#42f5c5);border-radius:999px;box-shadow:0 0 18px rgba(66,245,197,.25)}}.stats{{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}}.stat{{background:#080e12;border:1px solid #142329;border-radius:11px;padding:10px}}.stat small{{display:block;color:#53686f;font-size:7px}}.stat b{{display:block;margin-top:5px;font-size:10px}}.stat.green b{{color:var(--a)}}.stat.blue b{{color:var(--b)}}.grid2{{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:10px}}.box{{background:#090f13;border:1px solid var(--line);border-radius:16px;padding:15px}}.box-title{{font-size:8px;color:#637980;letter-spacing:.1em}}.box strong{{display:block;font-size:15px;margin-top:9px}}.url{{display:flex;align-items:center;gap:7px;margin-top:10px;background:#060a0d;border:1px solid #1b2b31;border-radius:10px;padding:9px}}.url code{{flex:1;min-width:0;direction:ltr;text-align:left;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#86aaa1;font:9px monospace}}.copy{{border:1px solid #285b50;background:#0c211d;color:var(--a);border-radius:9px;padding:8px 10px;font-size:8px;font-weight:900}}.formats{{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-top:10px}}.format{{padding:10px;border:1px solid #17282e;background:#080e12;border-radius:10px;text-align:center;color:#789099;font-size:8px}}.format i{{display:block;color:var(--a);font-size:17px;margin-bottom:4px}}.bottom{{margin-top:12px;display:flex;justify-content:space-between;color:#4f636b;font-size:8px}}.toast{{position:fixed;left:50%;bottom:18px;transform:translate(-50%,120px);transition:.25s;background:#0b1816;border:1px solid #2b5e53;color:#c9fff2;padding:10px 14px;border-radius:10px;font-size:9px;z-index:9}}.toast.show{{transform:translate(-50%,0)}}@media(max-width:470px){{.wrap{{width:min(100% - 14px,720px);padding-top:10px}}.passport{{padding:17px;border-radius:22px}}.stats{{grid-template-columns:1fr 1fr}}.stats .stat:last-child{{grid-column:1/-1}}.grid2{{grid-template-columns:1fr}}.head{{align-items:center}}h1{{font-size:20px}}}}
</style></head><body><main class="wrap"><header class="top"><div class="brand"><div class="mark">A</div><div><b>ALISON</b><small>ACCESS PASSPORT</small></div></div><div class="live {'off' if not active else ''}">● {status}</div></header>
<section class="passport"><div class="head"><div class="identity"><div class="avatar">{initial}</div><div><div class="eyebrow">IDENTITY</div><h1>{safe_label}</h1><div class="proto">{safe_protocol}</div></div></div><span class="state {status_class}">{status}</span></div><div class="notice">{'اشتراک فعال است و آماده اتصال می‌باشد.' if active else 'این دسترسی در حال حاضر فعال نیست.'}</div><div class="rule"></div><div class="usage-head"><b>TRAFFIC ALLOCATION</b><span>{pct}%</span></div><div class="bar"><div class="fill"></div></div><div class="stats"><div class="stat green"><small>USED</small><b>{escape_html(fmt_bytes(used))}</b></div><div class="stat blue"><small>REMAINING</small><b>{escape_html(fmt_bytes(remaining) if remaining is not None else 'نامحدود')}</b></div><div class="stat"><small>LIMIT</small><b>{escape_html(fmt_bytes(limit) if limit else 'نامحدود')}</b></div></div></section>
<section class="grid2"><div class="box"><div class="box-title">EXPIRATION</div><strong>{escape_html(expires[:19])}</strong></div><div class="box"><div class="box-title">LIVE SESSIONS</div><strong>{active_sessions} <small style="color:#60757c;font-size:8px">({active_people} IP)</small></strong></div></section>
<section class="box" style="margin-top:10px"><div class="box-title">SUBSCRIPTION ENDPOINT</div><div class="url"><code id="url">{safe_url}</code><button class="copy" onclick="copySub()">COPY</button></div><div class="formats"><div class="format"><i class="ti ti-brand-vlc"></i>V2RAY</div><div class="format"><i class="ti ti-arrows-transfer-up"></i>CLASH</div><div class="format"><i class="ti ti-box"></i>SING-BOX</div></div></section>
<div class="bottom"><span>ALISON NEXUS · ACCESS LAYER</span><span>SECURE CONNECTION</span></div></main><div class="toast" id="toast">Copied</div><script>function copySub(){{navigator.clipboard?.writeText(document.getElementById('url').textContent);const t=document.getElementById('toast');t.classList.add('show');setTimeout(()=>t.classList.remove('show'),1500);try{{const c=new AudioContext(),o=c.createOscillator(),g=c.createGain();o.frequency.value=740;g.gain.value=.025;o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+.07)}}catch(e){{}}}}</script></body></html>'''
